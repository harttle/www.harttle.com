...
> 更多 Vim 变量赋值和引用的细节，可参考这篇文章：[Vim 中的变量赋值、引用与作用域](https://www.harttle.com/2017/01/30/variables-in-vim.html)。
...
比如 [在 Vim 中优雅地查找和替换](https://www.harttle.com/2016/08/08/vim-search-in-file.html) 中介绍过可以用 `*`（向后），`#`（向前），`g*`（不切词）等命令来搜索光标所在的词搜索光标所在的词。
...
[使用 Vim 寄存器](https://www.harttle.com/2016/07/25/vim-registers.html)
