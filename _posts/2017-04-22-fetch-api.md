---
title: Fetch API 入门：替代繁琐的 AJAX
tags: CORS Cookie 跨域 Service-Worker
---
...
这意味着 `harttle.org` 的页面请求 `www.harttle.com` 中的资源时，
`harttle.org` 的 Service Worker 会收到 `fetch` 事件，但 `www.harttle.com` 的 Service Worker 不会
...
```javascript
fetch('https://www.harttle.com/foo.jpg', {mode: 'no-cors'}).then(res => {
    if (!res.ok) return;
    caches.open('harttle').then(cache => {
        cache.put('https://www.harttle.com/foo.jpg', res)
    })
})
```
...
```javascript
fetch('https://www.harttle.com', {credentials: 'same-origin'}).then(res => {
    // 处理 Response
})
```
