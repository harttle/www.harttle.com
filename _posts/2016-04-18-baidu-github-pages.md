...
并设置域名解析百度线路来的www.harttle.com到该镜像。
如今百度已经可以正常收录www.harttle.com的文章了，撰文在此希望能帮到别人。
...
首先在Coding.net的Pages页面绑定一个自定义域名：www.harttle.com。
然后在域名解析控制台中，将百度线路来的www.harttle.com解析到pages.coding.me：
...
这样从百度线路来的请求都会被解析到pages.coding.me，在HTTP请求头中仍然包含www.harttle.com主机的信息。
因为在Coding.net中设置了绑定www.harttle.com，所以Coding.net会很配合地接受该HTTP请求。
...
通过上述渠道访问站点的用户应当重定向到www.harttle.com，同时百度Spider不应被重定向。
解决这个问题很简单，在coding-pages分支中的HTML Head中添加脚本的重定向即可：

```html
<script>
  location.href='https://www.harttle.com' + location.pathname;
</script>
```
