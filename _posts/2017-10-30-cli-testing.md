...
it('should print author', function (done) {
  exec('./bin/cli --author',
    (err, stdout, stderr) => err ? done(err) : expect(stdout).to.equal('https://www.harttle.com')
  )
})
```
...
it('should print author', function () {
  ...
  return expect(p).to.eventually.equal('https://www.harttle.com')
})
```
...
在此 [Harttle](https://www.harttle.com) 介绍两种方式来初始化用于测试的工作区。
